...
class Table:
    ...
    async def select_all(self, *, mode: str="one"):
        async with self.pool.acquire() as connection:
            async with connection.cursor() as cursor:
                await cursor.execute("SELECT * FROM {};".format(self.name))
                if mode == "one":
                    return await cursor.fetchone()
                elif mode == "all":
                    return await cursor.fetchall()

    async def select(self, *, mode: str="one", **kwargs):
        async with self.pool.acquire() as connection:
            async with connection.cursor() as cursor:
                await cursor.execute("SELECT * FROM {} WHERE{};".format(
                    self.name, "AND ".join("
                ))
